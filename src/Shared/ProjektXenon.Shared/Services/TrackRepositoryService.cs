...
public class TrackRepositoryService
{
    ...
    public async Task<IReadOnlyList<MediaItem>?> GetFavoritesAsync()
    {
        ...
        {
            await using var json = File.OpenRead(AppDataPath + "/data.json");
            var list = await JsonSerializer.DeserializeAsync<List<MediaItem>>(json);
            if (list != null)
                return new ReadOnlyCollection<MediaItem>(list);
        }
    ...
    {
        if (File.Exists(AppDataPath + "/playlists.json"))
        {
            var json = File.OpenRead(AppDataPath + "/playlists.json");
    ...
    public async Task<IReadOnlyList<MediaItem>?> GetTracksAsync()
    {
    ...
    public async Task<IReadOnlyList<MediaItem>?> SearchAsync(string text)
    {
        var config = new Configuration().WithDefaultLoader();
        var context = BrowsingContext.New(config);
    ...
    public async Task ChangeFavoritesAsync(IEnumerable<MediaItem> media)
    {
        var serialized =
            JsonSerializer.Serialize(media, new JsonSerializerOptions() { WriteIndented = true, IndentSize = 4 });
        await File.WriteAllTextAsync(AppDataPath + "/data.json", serialized);
        FavoritesChanged?.Invoke(this, EventArgs.Empty);
    }

    private async Task<MediaItem> CreateMediaItem(IElement trackItem)
    {
